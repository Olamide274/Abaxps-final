    ...
    <>
      <section className="relative pt-44 pb-16 bg-ink text-cream overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 lg:px-10">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="font-display text-4xl md:text-6xl leading-[1.05]">
            {post.title}
          </motion.h1>
      ...
      <section className="py-20 lg:py-28">
        <article className="max-w-3xl mx-auto px-6 lg:px-10 space-y-7 text-lg leading-relaxed text-foreground/90">
          {post.content.map((para: string, i: number) => <p key={i}>{para}</p>)}
