...
function Chatbot() {
  ...
  const startChat = () => {
    ...
    setIntakeError("");
    setMessages([{ from: "bot", text: `Hello ${intake.name}! Welcome to Abax Professional Services. How can we help you today?` }]);
    setStage("chat");
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3">
          ...
          >
            <div className="bg-[#1B3FA0] text-white px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {stage === "chat" && <button onClick={reset} className="text-white/60 hover:text-white text-[10px] uppercase tracking-wide">Reset</button>}
            ...
            {stage === "intake" ? (
              <div className="p-5 space-y-4 bg-[#F8FAFF]">
                <p className="text-xs text-[#4A65B8] leading-relaxed">Before we chat, please share a few details so we can assist you better.</p>
                {[
                ...
                <button onClick={startChat} className="w-full bg-[#1B3FA0] text-white text-sm font-semibold py-2.5 rounded-lg hover:bg-[#F5C518] hover:text-[#1B3FA0] transition-colors">Start Chat →</button>
              </div>
            ) : (
              <>
                <div className="h-64 overflow-y-auto p-4 space-y-3 bg-[#F8FAFF]">
                  {messages.map((m, i) => (
                    <div key={i} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[82%] px-3 py-2 rounded-xl text-sm leading-relaxed ${m.from === "user" ? "bg-[#1B3FA0] text-white" : "bg-white border border-[#C8D4F5] text-[#1B3FA0]"}`}>{m.text}</div>
                    </div>
                  ))}
                </div>
                <div className="p-3 border-t border-[#C8D4F5] flex gap-2">
                  <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Type a message..." className="flex-1 text-sm border border-[#C8D4F5] rounded-full px-4 py-2 outline-none focus:border-[#1B3FA0]" />
      ...
      <button onClick={() => setOpen((v) => !v)} className="w-14 h-14 rounded-full bg-[#1B3FA0] text-white shadow-xl flex items-center justify-center hover:bg-[#F5C518] hover:text-[#1B3FA0] transition-colors" aria-label="Chat with us">
        {open
