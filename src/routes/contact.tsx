        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16">
          <div className="lg:col-span-7">
              ...
              {info.map((item, i) => {
                ...
                const body = (
                  <div className="bg-ink p-6 lg:p-7 h-full group hover:bg-ink/60 transition-colors">
                    ...
                    <div className="text-base lg:text-lg whitespace-pre-line leading-snug group-hover:text-gold transition-colors">{item.value}</div>
                  </div>
                );
                return (
            ...
            <div className="mt-10 flex items-center gap-4">
              <a href="https://www.linkedin.com/company/abax-professional-services" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 px-5 py-3 border border-cream/20 rounded-full text-sm hover:bg-gold hover:text-gold-foreground hover:border-gold transition-colors">
            ...
            {submitted ? (
              <div className="flex flex-col items-center justify-center py-12 text-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-cream/60 text-sm leading-relaxed">Thank you for reaching out. We'll respond within one business day.</p>
                </div>
                <button onClick={() => setSubmitted(false)} className="mt-4 text-xs uppercase tracking-[0.2em] text-gold border-b border-gold/40 pb-0.5 hover:border-gold transition-colors">Send another message</button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} noValidate className="space-y-6">
                ...
                {[
                  ...
                  <div key={f.id}>
                    ...
                    <input id={f.id} name={f.id} type={f.type} required={f.required} maxLength={200} className="w-full bg-transparent border-b border-cream/20 focus:border-gold outline-none py-2.5 text-cream placeholder:text-cream/30 transition-colors" />
                  </div>
                ))}
                <div>
                  <label htmlFor="msg" className="block text-[10px] uppercase tracking-[0.3em] text-cream/50 mb-2">Message<span className="text-gold"> *</span></label>
                  <textarea id="msg" name="msg" rows={5} required maxLength={4000} className="w-full bg-transparent border-b border-cream/20 focus:border-gold outline-none py-2.5 text-cream placeholder:text-cream/30 transition-colors resize-none" />
                </div>
                {error && <p className="text-red-400 text-xs leading-relaxed">{error}</p>}
                <button type="submit" disabled={submitting} className="w-full inline-flex items-center justify-center gap-3 bg-gold text-gold-foreground px-6 py-4 rounded-full text-sm font-medium hover:shadow-gold transition-all disabled:opacity-60 disabled:cursor-not-allowed">
                  {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : <>Send message <span aria-hidden>→</span></>}
                </button>
                <p className="text-[11px] text-cream/40 leading-relaxed">Your message is sent directly to our team. By submitting, you agree we may contact you regarding your enquiry.</p>
              </form>
            )}
          </motion.div>
        </div>
