...
export function Hero() {
  ...
  return (
    <>
      ...
      <section className="bg-white border-b border-[#C8D4F5] py-10">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6">
            {/* Image logos */}
            {[
              { src: icanLogo, alt: "ICAN – Fellow Members", sub: "Fellow Members" },
              { src: citnLogo, alt: "CITN – Fellow Members", sub: "Fellow Members" },
              { src: msiLogo,  alt: "MSI – Global Alliance",  sub: "Global Alliance" },
              { src: frcLogo,  alt: "FRC – Registered Firm",  sub: "Registered Firm" },
              { src: ndpcLogo, alt: "NDPC – Registered",      sub: "Registered" },
            ].map((p) => (
              <div key={p.alt} className="flex flex-col items-center justify-center gap-2 px-5 py-4 border border-[#C8D4F5] rounded-lg bg-[#EEF2FF] hover:border-[#1B3FA0] hover:shadow-sm transition-all group min-w-[100px]">
                <img src={p.src} alt={p.alt} className="h-10 w-auto object-contain" />
                <span className="text-[9px] uppercase tracking-wide text-muted-foreground text-center">{p.sub}</span>
              </div>
            ))}
            {/* Text-only logos for those without images */}
